...
foreach pair of local pairs {
    foreach grp of local pair {
        ...
        if r(N) == 0 | r(min) == r(max) {
            di as error "run_age_nih: `grp' degenerate -- aborting."
            exit 459
        }
...
postfile `ph' str30 yvar str24 grp str12 split_type ///
    ...
    using "../temp/age_nih_results", replace
...
foreach yvar of local yvars {
    ...
    foreach pair of local pairs {
        ...
        di as text _n "==== pooled DiD PPML: `yvar' `g1' vs `g2' ===="
        cap noi ppmlhdfe `yvar' Z_`g1' Z_`g2' S_`g1' S_`g2' PT_`g1', ///
            absorb(`fes') vce(cluster `vce_cl')
        if _rc {
            di as error "run_age_nih: pooled DiD `yvar' `g1'/`g2' failed; skipping."
            continue
        }
        local Nppml  = e(N)
        local r2ppml = e(r2_p)
...
use ../temp/age_nih_results, clear
