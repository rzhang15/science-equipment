...
out and isolated PIs still contribute (as low-similarity pairs). Reads the curve
at the panel's own max_sim quantiles, integrates it over the panel's realized
max_sim distribution to get the expected measurement error in imputed exposure,
and derives a similarity cutoff and a choice of K / neighbour weights from the
fitted model rather than from holdout accuracy. Inference is a PI-level cluster
bootstrap. Complements 2.1 and `5b_holdout_by_sim.py`, which condition on the
anchor set's own geometry.
