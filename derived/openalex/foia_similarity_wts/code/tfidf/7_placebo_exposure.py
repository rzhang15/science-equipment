"""
Placebo exposure: assign each non-FOIA universe PI an exposure value drawn at
random from the FOIA PIs' own empirical distribution, ignoring text similarity.

Real pipeline:    exposure_ss_i = (W @ S @ g)_i        -- neighbors carry the signal
Placebo:          exposure_ss_i = z_{j(i)}, j(i) ~ Unif(FOIA PIs)

(exposure, mkt_spend_shr) are drawn as a PI-level PAIR from the same FOIA row,
so the share control keeps its real relationship to the placebo regressor.

FOIA PIs' own rows are left at their real imputed values: analysis.do overwrites
them with observed exposure anyway ("replace imputed = exposure"), so the placebo
only moves the imputed (non-FOIA) PIs -- which is the object being tested.

Modes
-----
foia     (default) resample whole FOIA rows with replacement.
         Marginal = observed FOIA distribution (sd ~ .049 at hc), which is WIDER
         than the real imputed regressor (sd ~ .033): KNN averaging shrinks
         dispersion. Placebo betas are therefore not scale-comparable to the
         real spec without standardizing.
permute  randomly permute the REAL imputed rows across non-FOIA PIs.
         Marginal is exactly the real regressor's; only the PI<->exposure link
         is broken. Use this when the placebo beta must be scale-comparable.

Everything else about the run is held fixed: same universe roster, same max_sim
(so WEIGHT_MSIM and HISIM_PCT still select on real similarity), same observed
exposure for FOIA PIs.

Outputs (../../output/), drop-in for analysis.do:
    final_imputed_shift_share_{version}{filter}_plc{NN}.csv     (mode foia)
    final_imputed_shift_share_{version}{filter}_plcp{NN}.csv    (mode permute)
    placebo_exposure_summary_{version}{filter}.csv              per-draw moments

Run each draw by setting, in analysis/reduced_form/code/analysis.do:
    global EXPOSURE_VERSION "hc"
    global EXPOSURE_FILTER  "_cf_k3_plc01"
"""
