...
total_batches <- length(split_pmid)
...
for (q in start_batch:end_batch) {
  ...
  if(length(batch_pmids) == 0) next
  works_from_pmids <- tryCatch({
    oa_fetch(
      entity = "works",
      api_key = "01bd3ab8d66ee5d53d209f63f2dea37d",
      ids.pmid = batch_pmids, 
      output = "list"
    )
  }, error = function(e) {
  ...
  if (is.null(works_from_pmids) || length(works_from_pmids) == 0) next
  meta_df <- extract_article_meta(works_from_pmids)
  authors_df <- extract_authors_long(works_from_pmids)
  ...
  write_csv(final_df, paste0("../output/openalex_authors", as.character(q), ".csv"))
  
  mesh_terms <- map(works_from_pmids, function(article) {
    if (length(article[["mesh"]]) == 0) return(NULL)
    tibble(
      id = str_remove(article[["id"]], "https://openalex.org/"),
      which_mesh = seq_along(article[["mesh"]]),
      term = map_chr(article[["mesh"]], "descriptor_name"),
      is_major_topic = map_lgl(article[["mesh"]], "is_major_topic"),
      qualifier_name = map_chr(article[["mesh"]], ~ pluck(.x, "qualifier_name", .default = ""))
    )
  }) %>% list_rbind()
